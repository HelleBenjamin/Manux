...
void execute(VirtZ80 *cpu) {
  while (!cpu->halt) {
    if (cpu->pc >= 0x0100 && cpu->pc < 0xb000) break;
    cpu->r += 1;
    printState(cpu);
...
void MainInstruction(VirtZ80 *cpu) {
  ...
  switch (opcode) {
    ...
    case 0x32: // LD (nn), A
      addr = fWord(cpu);
      cpu->memory[addr] = getHigh(cpu->af);
      cpu->cycles += 13;
      break;
    case 0x33: // INC SP
      cpu->sp += 1;
    ...
    case 0x3A: // LD A, (nn)
      addr = fWord(cpu);
      loadHigh(&cpu->af, cpu->memory[addr]);
      cpu->cycles += 13;
      break;
    case 0x3B: // DEC SP
      cpu->sp -= 1;
...
void MiscInstruction(VirtZ80 *cpu) {
  ...
  switch (opcode) {
    ...
    case 0x56: // IM 1
      ...
      break;
    case 0x57: // LD A, I
      loadHigh(&cpu->af, cpu->i);
      cpu->cycles += 9;
      break;
    case 0x58: // IN E, (C)
      loadLow(&cpu->de, InputHandler(getLow(cpu->de)));
      cpu->cycles += 12;
      break;
    case 0x59: // OUT (C), E
      OutputHandler(getLow(cpu->de), getHigh(cpu->de));
    ...
    case 0x73: // LD (nn), SP
      ...
      break;
    case 0x78: // IN A, (C)
      loadHigh(&cpu->af, InputHandler(getLow(cpu->bc)));
      cpu->cycles += 12;
      break;
    case 0x79: // OUT (C), A
      OutputHandler(getLow(cpu->bc), getHigh(cpu->af));
      cpu->cycles += 12;
      break;
    case 0x7A: // ADC HL, SP
    ...
    case 0xA1: // CPI
      ...
      break;
    case 0xA2: // INI
      cpu->memory[cpu->hl] = InputHandler(getLow(cpu->bc));
      cpu->hl += 1;
      cpu->bc -= 1;
      if (cpu->bc == 0) {
        cpu->af |= FLAG_P;
      ...
      break;
    case 0xA3: // OUTI
      OutputHandler(getLow(cpu->bc), cpu->memory[cpu->hl]);
      cpu->hl += 1;
      cpu->bc -= 1;
      if (cpu->bc == 0) {
        cpu->af |= FLAG_P;
...
void IxInstruction(VirtZ80 *cpu) {
  ...
  switch (opcode) {
    case 0x09: // ADD IX, BC
      alu16(cpu, &cpu->ix, cpu->bc, ALU_OP_ADD);
      cpu->cycles += 8;
      break;
    case 0x19: // ADD IX, DE
      alu16(cpu, &cpu->ix, cpu->de, ALU_OP_ADD);
      cpu->cycles += 8;
      break;
    case 0x21: // LD IX, nn
      cpu->ix = fWord(cpu);
    ...
    case 0x22: // LD (nn), IX
      addr = fWord(cpu);
      cpu->memory[addr] = getLow(cpu->ix);
      cpu->memory[addr + 1] = getHigh(cpu->ix);
      cpu->cycles += 20;
      break;
    case 0x23: // INC IX
      cpu->ix += 1;
    ...
    case 0x2A: // LD IX, (nn)
      addr = fWord(cpu);
      cpu->ix = cpu->memory[addr] | (cpu->memory[addr + 1] << 8);
      cpu->cycles += 20;
      break;
    case 0x2B: // DEC IX
      cpu->ix -= 1;
      ...
      break;
    case 0x34: // INC (IX+d)
      addr = cpu->ix + (int8_t)fByte(cpu);
      temp1 = cpu->memory[addr];
      alu8(cpu, &temp1, 1, ALU_OP_ADD);
      cpu->memory[addr] = temp1;
      cpu->cycles += 12;
      break;
    case 0x35: // DEC (IX+d)
      addr = cpu->ix + (int8_t)fByte(cpu);
      temp1 = cpu->memory[addr];
      alu8(cpu, &temp1, 1, ALU_OP_SUB);
      cpu->memory[addr] = temp1;
      cpu->cycles += 12;
      break;
    case 0x39: // ADD IX, SP
      alu16(cpu, &cpu->ix, cpu->sp, ALU_OP_ADD);
      cpu->cycles += 8;
      break;
    ...
    case 0xE1: // POP IX
      ...
      break;
    case 0xE3: // EX (SP), IX
      temp1 = getHigh(cpu->sp);
      loadHigh(&cpu->sp, getHigh(cpu->ix));
      loadHigh(&cpu->ix, temp1);
      cpu->cycles += 4;
      break;
    case 0xE5: // PUSH IX
      push(cpu, cpu->ix);
    ...
    case 0xE9: // JP (IX)
      ...
      break;
    default:
      printf("Unknown IX opcode: 0x%02x\n", opcode);
      return;
  }
}
